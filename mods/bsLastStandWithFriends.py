...
def bsGetGames():
    return [LastStandwithFriendsGame]
...
class LastStandwithFriendsGame(bs.TeamGameActivity):
    ...
    def getInstanceDescription(self):
        return 'Play The Last Stand with Friends now! by AbhinaY!' if isinstance(self.getSession(),bs.TeamsSession) else 'Play The Last Stand with Friends now! by AbhinaY!'
    ...
    def _startBotUpdates(self):
        self._botUpdateInterval = (3300 - 300*(len(self.players))) if self.settings['Epic Mode'] else (7800 - 300*(len(self.players)))
        self._updateBots()
        self._updateBots()
    ...
    def _allPlayersDead(self):
        return all(player.gameData['lives'] <= 0 for player in self.players if player.exists())
